...
// Event colors palette (Google Agenda style)
...
const TRANSLATIONS = {
  fr:{
    ...
    errWrongCredentials:"Nom ou mot de passe incorrect.",
    ...
    docsBy:"Par",docsDelete:"Supprimer",
  },
  en:{
    ...
    errWrongCredentials:"Incorrect name or password.",
...
const initUsers = () => {
  if (!store.get("users")) store.set("users",[{id:"admin",name:"Formateur",role:"admin",password:"admin123",avatar:"F",color:"#5865f2"}]);
};
const getUsers    = () => store.get("users")    || [];
const saveUsers   = u  => store.set("users",u);
const getMessages = () => store.get("messages") || [];
const saveMessages= m  => store.set("messages",m);
const getSchedule = () => store.get("schedule") || [];
const saveSchedule= s  => store.set("schedule",s);
const getInOut    = () => store.get("inout")    || [];
const saveInOut   = i  => store.set("inout",i);
const getDocs     = () => store.get("docs")     || [];
const saveDocs    = d  => store.set("docs",d);
...
function Inp({value,onChange,placeholder,type="text",style:s={}}){
  return <input type={type} value={value} onChange={onChange} placeholder={placeholder} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,padding:"9px 12px",color:C.text,fontSize:14,outline:"none",width:"100%",...s}}/>;
}
...
function LoginScreen({onLogin,onRegister,lang,setLang}){
  ...
  const submit=()=>{
    const u=getUsers().find(u=>u.name.toLowerCase()===name.trim().toLowerCase()&&u.password===pass);
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg}}>
      <div style={{background:C.surface,borderRadius:12,padding:36,width:380,boxShadow:"0 8px 32px #0006"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <Btn onClick={submit} style={{marginTop:4}}>{t.loginBtn}</Btn>
...
function RegisterScreen({onBack,onLogin,lang,setLang}){
  const t=TRANSLATIONS[lang];
  const [name,setName]=useState(""); const [pass,setPass]=useState(""); const [pass2,setPass2]=useState(""); const [err,setErr]=useState("");
  const submit=()=>{
    ...
    if(users.find(u=>u.name.toLowerCase()===name.trim().toLowerCase())) return setErr(t.errNameTaken);
    const nu={id:Date.now().toString(),name:name.trim(),role:"student",password:pass,avatar:name[0],color:randomColor()};
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg}}>
      <div style={{background:C.surface,borderRadius:12,padding:36,width:380,boxShadow:"0 8px 32px #0006"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          {err&&<p style={{color:C.red,fontSize:13}}>{err}</p>}
          <Btn onClick={submit} style={{marginTop:4}}>{t.createBtn}</Btn>
...
function ScheduleTab({user,lang}){
  ...
  const [items,setItems]=useState(getSchedule());
  ...
  const addEvent=(ev)=>{
    const updated=[...items,ev].sort((a,b)=>a.date.localeCompare(b.date)||(a.timeStart||"").localeCompare(b.timeStart||""));
    setItems(updated); saveSchedule(updated);
  };
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:C.bg}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,padding:"14px 20px",borderBottom:`1px solid ${C.border}`,background:C.panel,flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:4}}>
          <button onClick={prevMonth} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.text,width:32,height:32,fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
          <button onClick={nextMonth} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:6,color:C.text,width:32,height:32,fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
        </div>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",background:C.panel,borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
            {t.calDays.map(d=>(
              <div key={d} style={{textAlign:"center",padding:"8px 4px",fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.5}}>{d}</div>
            ))}
          </div>
          {/* Day cells */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gridAutoRows:"minmax(80px,1fr)",flex:1,overflowY:"auto",gap:0}}>
            {cells.map((day,i)=>{
              ...
              return(
                <div key={dk} onClick={()=>setSelectedDay(isSelected?null:dk)}
                  style={{borderRight:`1px solid ${C.border}`,borderBottom:`1px solid ${C.border}`,padding:"6px 4px",cursor:"pointer",background:isSelected?C.accent+"22":C.bg,transition:"background .1s",minHeight:80,overflow:"hidden"}}>
                  <div style={{display:"flex",justifyContent:"center",marginBottom:3}}>
                    <span style={{width:26,height:26,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:isToday?800:500,background:isToday?C.accent:"transparent",color:isToday?"#fff":isSelected?C.accent:C.text}}>
                      {day}
                    </span>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:2}}>
                    {dayEvents.slice(0,3).map(ev=>(
                      <div key={ev.id} onClick={e=>{e.stopPropagation();setModal({date:dk,event:ev});}}
                        style={{background:ev.color||C.accent,borderRadius:3,padding:"1px 5px",fontSize:11,fontWeight:600,color:"#fff",overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",cursor:"pointer"}}>
      ...
      {modal&&(
        <EventModal
          t={t} lang={lang} modal={modal} user={user} isAdmin={isAdmin}
          onSave={ev=>{addEvent(ev);setModal(null);setSelectedDay(modal.date);}}
...
function EventModal({t,lang,modal,user,isAdmin,onSave,onDelete,onClose}){
  ...
  const save=()=>{
    ...
    const newEv={
      ...
      date:modal.date,title:title.trim(),desc:desc.trim(),
      timeStart,timeEnd,color,createdBy:user.name,
    };
    onSave(newEv);
  };
        ...
        {!viewing&&<>
          <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <Inp value={timeStart} onChange={e=>setTimeStart(e.target.value)} type="time" placeholder={t.calTimeStart}/>
              <Inp value={timeEnd}   onChange={e=>setTimeEnd(e.target.value)}   type="time" placeholder={t.calTimeEnd}/>
            </div>
            ...
            <div>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {EVENT_COLORS.map(c=>(
                  <button key={c} onClick={()=>setColor(c)} style={{width:24,height:24,borderRadius:"50%",background:c,border:color===c?"3px solid #fff":"3px solid transparent",cursor:"pointer",outline:"none"}}/>
                ))}
              </div>
            </div>
          </div>
          <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
            ...
            <Btn onClick={save}>{t.calSave}</Btn>
          </div>
        </>}
        ...
        {viewing&&<>
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <p style={{color:C.muted,fontSize:14}}>{ev.timeStart||"—"}{ev.timeEnd?` – ${ev.timeEnd}`:""}</p>
            </div>
            {ev.desc&&<div style={{display:"flex",alignItems:"flex-start",gap:8}}><span style={{fontSize:16}}>📝</span><p style={{color:C.muted,fontSize:14}}>{ev.desc}</p></div>}
            <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>👤</span><p style={{color:C.muted,fontSize:14}}>{t.calBy} {ev.createdBy}</p></div>
          </div>
          <Btn variant="ghost" onClick={onClose}>{t.calCancel}</Btn>
        </>}
      </div>
...
function ChatTab({user,lang}){
  ...
  const send=()=>{
    if(!text.trim()) return;
    const msg={id:Date.now().toString(),userId:user.id,userName:user.name,userColor:user.color,text:text.trim(),at:new Date().toISOString()};
    const u=[...messages,msg]; setMessages(u); saveMessages(u); setText("");
  };
        ...
        {grouped.map((g,gi)=>(
          <div key={gi} style={{display:"flex",gap:12,padding:"6px 0"}}>
            ...
            <div style={{flex:1}}>
              ...
              {g.messages.map(m=><p key={m.id} style={{fontSize:15,lineHeight:1.5,color:C.text,wordBreak:"break-word"}}>{m.text}</p>)}
            </div>
          </div>
        ))}
        <div ref={bottomRef}/>
...
function InOutTab({user,lang}){
  const t=TRANSLATIONS[lang];
  const [log,setLog]=useState(getInOut());
  const [reason,setReason]=useState("");
  const myStatus=()=>{const my=log.filter(e=>e.userId===user.id);return my.length>0?my[my.length-1].type:null;};
  const toggle=type=>{
    const e={id:Date.now().toString(),userId:user.id,userName:user.name,userColor:user.color,type,reason:reason.trim(),at:new Date().toISOString()};
    const u=[...log,e]; setLog(u); saveInOut(u); setReason("");
  };
  ...
  const todayLog=log.filter(e=>e.at.startsWith(new Date().toISOString().split("T")[0])).slice().reverse();
  return(
...
function DocsTab({user,lang}){
  ...
  const [caption,setCaption]=useState(""); const [preview,setPreview]=useState(null); const [lightbox,setLightbox]=useState(null);
  const fileRef=useRef(null);
  const isAdmin=user.role==="admin";
  const handleFile=e=>{
    const f=e.target.files[0]; if(!f) return;
    const r=new FileReader();
    r.onload=ev=>setPreview({src:ev.target.result,name:f.name});
    r.readAsDataURL(f);
  };
  const save=()=>{
    if(!preview) return;
    const doc={id:Date.now().toString(),src:preview.src,name:caption.trim()||preview.name,uploadedBy:user.name,userColor:user.color,at:new Date().toISOString()};
    const u=[doc,...docs]; setDocs(u); saveDocs(u); setPreview(null); setCaption("");
    if(fileRef.current) fileRef.current.value="";
  };
  const remove=id=>{const u=docs.filter(d=>d.id!==id);setDocs(u);saveDocs(u);};
  return(
    <div style={{padding:24,maxWidth:800,margin:"0 auto"}}>
        ...
        {preview&&(
          <div style={{marginTop:14}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:10}}>
              <Btn onClick={save}>{t.docsSave}</Btn>
      ...
      {lightbox&&(
        <div style={{position:"fixed",inset:0,background:"#000c",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999}} onClick={()=>setLightbox(null)}>
          <img src={lightbox.src} alt={lightbox.name} style={{maxWidth:"90vw",maxHeight:"85vh",borderRadius:10,boxShadow:"0 8px 40px #000a"}}/>
        </div>
      )}
    </div>
  );
...
export default function App(){
  ...
  const login=u=>{store.set("currentUser",u);setUser(u);setScreen("app");};
  const logout=()=>{store.set("currentUser",null);setUser(null);setScreen("login");};
  ...
  return(
    <>
      ...
      <div style={{display:"flex",height:"100vh",flexDirection:isMobile?"column":"row",overflow:"hidden"}}>
        ...
        {isMobile&&(
          <div style={{height:64,borderTop:`1px solid ${C.border}`,flexShrink:0}}>
            <Sidebar active={tab} onSelect={setTab} user={user} onLogout={logout} isMobile={true} lang={lang} setLang={changeLang} t={t}/>
          </div>
        )}
      </div>
    </>
